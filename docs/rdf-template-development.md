...
It also described hints and best practices to ensure that the resulting RDF file complies with the [RDF/Turtle syntax](https://www.w3.org/TR/turtle/ "RDF/Turtle Specification").
...
The query has to use a variable called `?uri` to bind the URIs of the queried terms and `?displayName` to bind labels for the queried terms (e.g. attached via rdfs:label).

Given an endpoint and a query, an autocompletion will be provided to form authors who want to use the semantic property.
If *Endpoint* and *Query* are intentionally left empty, no autocompletion will be provided.

A new template group can then be registered by simply adding its to the *templates* list.
Further configuration like which semantic properties it uses will be added in a separate file.

The following snippet shows a valid *rdfExportTemplateConfig.yml*-file:
...
Once the `config.yml` is defined, a few files have to be defined to describe how the RDF will be built during the export.
...
+ *.* - **String** - Base URI of the resulting RDF file
...
+ *locationList* - **List\<GeotraceElement>** - List of the collected locations. Each element exposes the fields:
  ...
  + *location* - **Location** - The collected location. For the fields that this object exposes, please refer to the [geolocationCell.ttl.mustache](#geolocationCell.ttl.mustache) section.